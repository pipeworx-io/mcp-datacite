...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'DataCite');
}
    ...
    inputSchema: {
      ...
      properties: {
        ...
        sort: {
          ...
          enum: ['relevance', 'recent', 'oldest', 'published', 'citations', 'downloads', 'views'],
        },
...
async function dcFetch<T>(path: string, params?: URLSearchParams): Promise<T> {
  ...
  const res = await pwFetch(url, { headers: { Accept: 'application/vnd.api+json' } });
...
async function searchDois(args: Record<string, unknown>) {
  ...
  const sortKey = typeof args.sort === 'string' ? args.sort : 'relevance';
  if (SORT_MAP[sortKey] && sortKey !== 'relevance') params.set('sort', SORT_MAP[sortKey]);
